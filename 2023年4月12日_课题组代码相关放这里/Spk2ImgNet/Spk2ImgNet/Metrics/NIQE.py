...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    size = tuple((int(img.shape[0]*0.5),int(img.shape[1]*0.5)))
    print(size)
    img2 = np.array(img2.resize(size, Image.BICUBIC))
...
if __name__ == "__main__":
    path = '/home/yangchen/PycharmProjects/MyResearch/256x256/EvalResults'
    fileNames = os.listdir(path)
    for name in fileNames:
        filePath = os.path.join(path, name)
        img = np.array(Image.open(filePath))
        print('NIQE of ref bikes image is: %0.3f' % niqe(img))

    
    # ref = np.array(Image.open('spikling-master/result/train-intv.png').convert('LA'))[:,:,0] # ref
